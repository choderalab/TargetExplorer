...
database_dir = os.path.join(clab.core.kinome_rootdir, 'database')
external_data_dir = os.path.join(database_dir, 'external-data')
...
retrieve_methods = {
'BindingDB' : clab.BindingDB.retrieve_all_BindingDB_data
}
...
def retrieve_external_data(gather_script_ID, forcedl=False, uniprot_search_string=None):
    ...
    local_datafile_datestamp = datetime.datetime.strptime(local_datafile_datestamp, clab.DB.datestamp_format_string)
