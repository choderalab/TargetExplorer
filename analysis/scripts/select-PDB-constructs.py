...
import sys, os, openpyxl, re, copy
...
for t in range(len(targets_data)):
    ...
    # ===========
    # rank the PDB constructs based firstly on the alignment score, and secondly on the number of aas outside the target domain sequence
    # ===========
    # dict_for_sorting structure: { 'sequence' : (alignment_score, num_aas_outside_target_domain), ... }
    dict_for_sorting = { x[1] : (aln_scores[i], num_aas_outside_target_domain[i]) for i, x in enumerate(PDB_construct_seqs_aligned) }
    # PDB_construct_seqs_aligned structure: [ ['PDB_ID', 'sequence'], ... ]
