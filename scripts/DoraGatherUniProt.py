...
def parse_arguments():
        ...
        argparser.add_argument(
            '--use_existing_data',
            help='Do not download a new UniProt document. Only works if an existing document is present.',
            action='store_true',
            default=False
        )
        argparser.add_argument(
            '--count_nonselected_domain_names',
            help='Count the number of occurrences of domain names which are not selected by the regex and write to the "selected_domain_names.txt" file (may take a little while)',
            action='store_true',
            default=False
        )
        argparser.add_argument(
